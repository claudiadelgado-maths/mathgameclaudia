...
function setupTerminalViewport() {
  ...
  if (window.visualViewport) {
    window.visualViewport.addEventListener("resize", updateTerminalViewport);
    window.visualViewport.addEventListener("scroll", updateTerminalViewport);
  }
}
...
async function initTerminal() {
  ...
  try {
    ...
    renderTerminal(loaded.data, loaded.fileName);
    status.textContent = `${loaded.data.publicaciones.length} publicación(es) cargada(s).`;
  } catch (error) {
    console.error("No se pudo cargar la terminal:", error);
    status.textContent = "No se pudo cargar el contenido mensual.";
    renderEmptyState("No fue posible leer el archivo mensual de la terminal.");
  }
}
...
function renderTerminal(data, fileName) {
